...
class HeimdallTests: XCTestCase {
  ...
  var privateTag: String = "com.example.heimdall.private.tests"
  var publicTag: String = "com.example.heimdall.tests"
  var destroyedPrivateTag: String = "com.example.heimdall.private.destroyed.tests"
  var destroyedPublicTag: String = "com.example.heimdall.destroyed.tests"

  override func setUp() {
    self.privateHeimdall = Heimdall(tagPrefix: privateTag, keySize: 2048)
    guard self.privateHeimdall != nil else {
      assert(false)
      return
    }
    let _ = self.privateHeimdall.regenerate(2048) // Make sure that nothing leaks between tests

    self.publicHeimdall = Heimdall(publicTag: publicTag, publicKeyData: self.privateHeimdall.publicKeyData()!)!

    self.destroyedPrivateHeimdall = Heimdall(tagPrefix: destroyedPrivateTag, keySize: 2048)!
    let _ = self.destroyedPrivateHeimdall.destroy()

    self.destroyedPublicHeimdall = Heimdall(publicTag: destroyedPublicTag, publicKeyData: self.privateHeimdall.publicKeyData()!)!
    let _ = self.destroyedPublicHeimdall.destroy()
  }

  override func tearDown() {
    let _ = self.publicHeimdall.destroy()
    let _ = self.privateHeimdall.destroy()
  }
